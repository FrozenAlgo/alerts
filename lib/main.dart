...
const Color kPrimaryCyan = Color(0xFF00E5FF);
const Color kDarkSlate = Color(0xFF0F172A);
const Color kGlassBase = Color(0xFF1E293B);
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(
          seedColor: const Color(0xFF00E5FF), // Electric Cyan
          brightness: Brightness.dark,
          surface: const Color(0xFF1E293B), // Glass panel base
        ),
      ),
...
/// =======================
/// 🔥 SPLASH SCREEN (UPDATED FOR AUTO-LOGIN)
/// =======================
  ...
  void initState() {
    ...
    Timer(const Duration(seconds: 5), () async {
      ...
      if (mounted) {
        ...
        if (mounted) {
          if (user != null) {
            // Valid token found -> Go straight to Dashboard
              ...
              children: [
                Row(
                    ...
                    children: [
                      ...
                      TextButton(
                          onPressed: () => DeviceController.removeDevice(widget.userId),
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                        ...
                        Column(children: [Icon(Icons.wifi, color: isOnline ? AppTheme.kPrimaryCyan : Colors.grey), Text("GPS", style: TextStyle(color: Colors.blueGrey.shade300, fontSize: 10)), Text(isOnline ? "CONNECTED" : "OFFLINE", style: TextStyle(color: isOnline ? AppTheme.kPrimaryCyan : Colors.grey, fontWeight: FontWeight.bold))])
                      ])
                    ],
                  ),
                ),
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (mounted) await NotificationService().sendEmergencyAlert(context, snap.docs.map((d) => d.data()).toList(), widget.userId, lat, lng);
                  },
                  child: Container(
                    width: double.infinity,
                    height: 60,
...
/// ==========================================
/// 📡 NOTIFICATION SERVICE (Real-Time Module)
/// ==========================================
class NotificationService {
  Future<void> sendEmergencyAlert(
    ...
    if (contacts.isNotEmpty) {
      String phone = contacts[0]['number'] ?? "";
      final String googleMapsUrl = "https://www.google.com/maps/search/?api=1&query=$lat,$lng";
      final String message = "EMERGENCY ALERT! I have been in an accident. My exact location: $googleMapsUrl";
...
class FullScreenEmergencyAlert extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // Deep dark-red background for emergency state
...
class _AlertsScreenState extends State<AlertsScreen> {
  bool _isDescending = true; // State for date filtering
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(_isDescending ? "Showing Newest First" : "Showing Oldest First"),
      ...
      body: StreamBuilder<cf.QuerySnapshot>(
        stream: cf.FirebaseFirestore.instance
            ...
            .orderBy('timestamp', descending: _isDescending) // Added dynamic sort
            .snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator(color: AppTheme.kPrimaryCyan));
...
class DetailedAlertView extends StatelessWidget {
  ...
  void _exportIncidentReport(BuildContext context, DateTime ts) {
    // Scaffold UI logic to simulate export.
    // For real PDF, integrate 'pdf' and 'printing' packages here.
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Incident Report Generated! Check your downloads folder."),
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _exportIncidentReport(context, ts),
          )
        ],
      ),
      body: SingleChildScrollView(
      ...
      body: ListView(
        ...
        children: [
          ...
          _settingsGroup([
            ...
            const Divider(height: 1, color: Colors.white10),
            _buildActionTile("Export Data", "Download your incident history", Icons.download_rounded, () {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Data Export Started. Check your email shortly."), backgroundColor: Colors.greenAccent));
            }),
            const Divider(height: 1, color: Colors.white10),
...
class DeviceController {
  ...
  /// Pairs a hardware device (ESP32) by verifying its serial and PIN against the Realtime Database.
  static Future<void> pairDevice(String inputSerial, String inputPin, String userId) async {
    try {
      ...
      if (snapshot.exists) {
        ...
        if (data['security_pin']?.toString() == inputPin) {
          ...
          debugPrint("DeviceController: Successfully paired serial: $inputSerial");
        } else {
      ...
      debugPrint("DeviceController Error: $e");
      rethrow; // Pass error back to UI for SnackBar feedback
    }
  }

  /// Unpairs the device by clearing the reference in the user's Firestore document.
  static Future<void> removeDevice(String userId) async {
    try {
      await FirebaseFirestore.instance.collection('users').doc(userId).update({
        'pairedDevice': FieldValue.delete(), // Using delete() is cleaner than setting to empty string
      });
      debugPrint("DeviceController: Device successfully removed for user: $userId");
    } catch (e) {
      debugPrint("DeviceController Error (Removing Device): $e");
      rethrow;
    }
  }
}
...
class _MapScreenState extends State<MapScreen> {
    ...
    markers = {
      Marker(
        ...
        infoWindow: const InfoWindow(
          ...
          snippet: 'High G-Force Impact',
        ),
        // Using a custom red icon to match the alert theme
  ...
  void _onMapCreated(GoogleMapController controller) {
    ...
    // Apply dark map style if you have a JSON file,
    // otherwise the Hybrid map type provides good contrast.
  }

  @override
  Widget build(BuildContext context) {
    final LatLng center = LatLng(widget.lat, widget.lng);
    ...
    return Scaffold(
      ...
      body: Container(
        decoration: BoxDecoration(
          border: Border.all(color: AppTheme.kPrimaryCyan.withOpacity(0.3), width: 2),
          borderRadius: BorderRadius.circular(20),
        ),
        child: ClipRRect(
          borderRadius: BorderRadius.circular(20),
          child: GoogleMap(
            ...
            markers: markers,
            mapType: MapType.hybrid, // Excellent for accident context
...
class _DeviceManagerPageState extends State<DeviceManagerPage> {
  ...
  void _simulateUpdateCheck() async {
  ...
  void _showUpToDateDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("OnAlert Firmware v1.0.0 is the latest version.", textAlign: TextAlign.center, style: TextStyle(color: Colors.blueGrey)),
        actions: [
          ...
          return ListView(
            ...
            children: [
              ...
              _actionTile(
                ...
                onTap: () => isPaired ? DeviceController.removeDevice(widget.userId) : _showPairingDialog(),
              ),
              _actionTile(
                ...
                onTap: _isCheckingUpdate ? null : _simulateUpdateCheck,
              ),
              _actionTile(
                title: "System Diagnostics",
                subtitle: "Run tests, view logs, simulate SOS",
                icon: Icons.bug_report_rounded,
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SystemTestingPage())),
              ),
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              try {
                await DeviceController.pairDevice(sCtrl.text.trim(), pCtrl.text.trim(), widget.userId);
                if (mounted) Navigator.pop(context);
...
/// ==========================================
class PermissionsManagerPage extends StatelessWidget {
            ...
            child: Column(
              children: [
                ...
                _buildPermissionTile(
                    ...
                    subtitle: "Vibrate during SOS countdown.",
                    value: true
                ),
              ],
            ),
  ...
  Widget _buildPermissionTile({required IconData icon, required String title, required String subtitle, required bool value}) {
...
class SystemTestingPage extends StatefulWidget {
  const SystemTestingPage({super.key});
    ...
    if (mounted) {
      showDialog(
          ...
          builder: (_) => AlertDialog(
            backgroundColor: AppTheme.kGlassBase,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: Colors.white.withOpacity(0.1))),
            title: const Row(children: [Icon(Icons.check_circle, color: Colors.greenAccent), SizedBox(width: 10), Text("Diagnostic Pass", style: TextStyle(color: Colors.white))]),
            content: const Text("Sensors: OK\nGPS: OK\nDatabase Connection: OK\nFCM Token: Valid", style: TextStyle(color: Colors.white70)),
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildTestCard(
            ...
            subtitle: "View raw incident payloads sent to Firebase.",
          ...
          _buildTestCard(
              ...
              onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Pinging Hardware... Latency: 42ms"), backgroundColor: Colors.green))
          ),
        ],
      ),
    );
